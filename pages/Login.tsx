...
const Login: React.FC<LoginProps> = ({ onToggleAuth }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2) Get profile row from "profiles" table (by auth user id)
      ...
      // If profile missing, create a minimal profile row so app logic works
      if (!finalProfile) {
        const defaultName = user.user_metadata?.full_name || email.split('@')[0];
        const avatar = `https://ui-avatars.com/api/?name=${encodeURIComponent(defaultName)}&background=random`;
        const insertPayload = {
          id: user.id,
          email,
          name: defaultName,
        ...
        if (insError) {
          console.error('Failed to create fallback profile:', insError);
        } else {
          finalProfile = insData;
        }
      }

      // 3) Ensure local store is updated with the latest profile from DB
      // Always call signup(...) so the app's in-memory store has up-to-date role/name/avatar etc.
      // signup in your store acts like "setCurrentUser" — it will overwrite existing entry.
      signup({
        name: finalProfile?.name ?? (user.user_metadata?.full_name || email.split('@')[0]),
        email: finalProfile?.email ?? email,
        password,
        phone: finalProfile?.phone ?? '',
        role: finalProfile?.role === 'admin' ? Role.ADMIN : Role.PATIENT,
        address: finalProfile?.address ?? undefined,
        avatar: finalProfile?.avatar_url ?? `https://ui-avatars.com/api/?name=${encodeURIComponent(finalProfile?.name ?? email.split('@')[0])}&background=random`
      });


      // success — your store change should update UI / redirect as your app does
    } catch (err) {
    ...
    <div className="min-h-screen flex bg-white">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-primary-700 to-primary-900 relative overflow-hidden flex-col justify-between p-12 text-white">
        ...
        <div className="relative z-10 text-sm text-primary-200 font-medium">
          © 2025 MediCore Healthcare Systems. <br />Innovating for a healthier tomorrow.
        </div>
      </div>
